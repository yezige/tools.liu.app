...
const fetchHead = async ({ url, opt = {} }) => {
  ...
  const response = await fetch(`${url}&head=1`, option)
...
const fetchDownload = async ({ url, name, chunkSize, poolLimit = 1 }) => {
  try {
    ...
    if (typeof head.success == 'undefined' || !head.success) {
      throw new Error(head.msg || '请刷新页面后重试')
    }
    const contentLength = head.data.length
    if (!contentLength) {
    ...
    const results = await asyncPool(poolLimit, [...new Array(chunks).keys()], async (i) => {
      ...
      try {
        return await getPartContent(url, start, end, i)
      } catch (err) {
        throw new Error(err)
      }
    })
...
const getPartContent = (url, start, end, i) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      fetch(`${url}&range_start=${start}&range_end=${end}`).then(async (res) => {
        if (res.status != 206) {
